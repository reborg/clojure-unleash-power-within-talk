...
(defmethod sexpr-to-ssa 'coreasync/pause
  [[_ expr]]
  (gen-plan
   [next-blk (add-block)
  ...
  ([f p state]
     (let [value (::value state)]
       (cond
        ...
        (extends? np/INotify (class value))
        (np/then value result
...
(defmacro async [& body]
  (binding [coreasync/*symbol-translations* '{await coreasync/pause}]
    `(coreasync/task-wrapper ~(coreasync/state-machine body))))

(comment
  (require '[coreasync :as ca])
  (require '[notifying-promise :as np])
  ...
  (coreasync/task-wrapper
    (let [locals (get-thread-bindings)]
      (fn state-machine
        ([]
         (state-machine
           (assoc {}
           ...
           (loop [state init]
             (case (:state state)
               ...
               :started (let [inst (:value state)]
                          (recur (assoc state :value inst :state :finished))))))))))
  )
